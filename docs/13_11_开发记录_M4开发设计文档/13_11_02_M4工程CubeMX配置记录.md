...
### 步骤 4：配置 TIM4_CH1 PWM（LED 调光）

> **已变更**：原为 TIM2_CH3 + PB10，因 TIM2 与 LCD 背光冲突（问题 #22），现改为 TIM4_CH1 + PB6。

1. 在芯片引脚图上找到 **PB6**，右键选择 **TIM4_CH1**
2. 左侧面板展开 **Timers → TIM4**
...
### 2.1 TIM4_CH1 — PWM 输出（LED 调光）

> **已变更**：原为 TIM2_CH3 + PB10，因 TIM2 与 LCD 背光冲突（问题 #22），现改为 TIM4_CH1 + PB6。

**用途**：通过 PB6 输出硬件 PWM 信号，直接驱动 HW-269 LED 模块实现无级调光。
...
| 参数 | 值 | 说明 |
|------|-----|------|
| Channel1 Mode | PWM Generation CH1 | PWM 输出模式 |
| Prescaler (PSC) | 199 | 分频系数 = 199+1 = 200 |
| Counter Period (ARR) | 999 | 自动重装值 = 999+1 = 1000 |
| Pulse (CCR1) | 0 | 初始占空比 = 0%（LED 灭） |
...
```
假设 TIM4 时钟源 = 200MHz（A7 配置）
PWM 频率 = 200MHz / (PSC+1) / (ARR+1) = 200MHz / 200 / 1000 = 1kHz
占空比 = Pulse / (ARR+1) × 100%
  - Pulse=0    → 0%（灭）
  - Pulse=500  → 50%（半亮）
  - Pulse=999  → 99.9%（最亮）
...
> **注意**：实际运行时 M4 由 A7 的 Linux 通过 remoteproc 加载，时钟由 A7 侧配置（通常 200MHz）。PSC=199 对应 200MHz 时钟下 1kHz PWM。

**为什么这样配置**：
- 1kHz PWM 频率足够高，人眼无法感知闪烁，实现平滑调光
- HW-269 LED 模块 data 引脚高电平点亮，PWM 占空比直接对应亮度
- 已通过 GPIO sysfs 验证 GPIO 可直接驱动 HW-269，无需 MOSFET
...
| 检查项 | 文件 | 结果 |
|--------|------|------|
| TIM4 PWM 初始化 | tim.c | ✅ PSC=199, ARR=999, CH1, PB6 AF2 |
...
| 外设初始化调用顺序 | main.c | ✅ GPIO → TIM3 → TIM4 |
...
### 6.3 填写 libmetal IRQ 实现（仅启用 FreeRTOS 时）

**文件**：`Middlewares/Third_Party/OpenAMP/libmetal/lib/system/freertos/template/sys.c`

CubeMX 添加 FreeRTOS 后会引入 libmetal FreeRTOS 后端，其中 `sys_irq_save_disable` 和 `sys_irq_restore_enable` 是空实现（模板文件），必须填写：
...
**原因**：空实现导致 OpenAMP 内部临界区保护无效，IPCC 中断可能在任意时刻打断 OpenAMP 操作，导致数据结构损坏。详见问题记录 #21。
...
### 7.1 TIM2 → 软件 PWM → TIM4 硬件 PWM（问题 #22）

**变更日期**：2025-07

**原因**：TIM2 与 LCD 背光（TIM2_CH1 PA15）冲突，M4 初始化 TIM2 导致屏幕熄灭。

**变更历史**：
- v1: PB10 改为 GPIO_Output，TIM3 中断软件 PWM（100Hz，有闪烁）
- v2: 尝试 PB6 + TIM4_CH1 硬件 PWM，但 PB6 物理位置不在 J1 可用引脚上，放弃
- v3: 回退 **PB10 + 高频软件 PWM**（TIM3 中断 50μs，500Hz，无闪烁）

**当前配置**：
- PB10 → GPIO_Output（标签 LED_PWM）
- TIM3: PSC=63, ARR=49, 中断周期 50μs
- 软件 PWM 40 级计数，周期 2ms = 500Hz
- TIM4 已移除

### 7.2 硬件 I2C1 → 软件 I2C（问题 #24）

**变更日期**：2025-07

**原因**：I2C1(PF14/PF15) 与触摸屏（Goodix）共享总线，M4 初始化 I2C1 破坏触摸屏通信。

**变更内容**：
- I2C1 完全禁用
- 注释 `/boot/uEnv.txt` 中的 `stm-fire-can.dtbo`，释放 PA11/PA12
- PA11(J1 pin 12) 配置为 GPIO Output Open Drain + Pull-up → 软件 I2C SDA
- PA12(J1 pin 11) 配置为 GPIO Output Open Drain + Pull-up → 软件 I2C SCL
- SHT30 接线从 J1 pin 13/14 改到 J1 pin 11/12

### 7.3 当前引脚分配总览

| 功能 | 引脚 | J1 Pin | 方式 |
|---|---|---|---|
| LED 补光灯 | **PB10** | pin 7 | **高频软件 PWM**（TIM3 中断 50μs，500Hz） |
| SHT30 SCL | PA12 | pin 11 | GPIO 软件 I2C |
| SHT30 SDA | PA11 | pin 12 | GPIO 软件 I2C |
| SHT30 VCC | - | pin 19 | 3V3 电源 |
| LED VCC | - | pin 23 | 5V 电源 |
| GND | - | pin 1 | 共地 |
| KEY1 冻结 | PB13 | KEY 接口 | GPIO 输入（TIM3 轮询） |
| KEY2 拍照 | PH7 | KEY 接口 | GPIO 输入（TIM3 轮询） |

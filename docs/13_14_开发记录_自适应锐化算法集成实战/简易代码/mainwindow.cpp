      ...
      m_currentMode(0)
{
    ...
    // M4 启动后 TIM2 初始化可能导致 LCD 背光熄灭，恢复背光
    QFile bl("/sys/class/backlight/lcd-backlight/brightness");
    if (bl.open(QIODevice::WriteOnly)) {
        bl.write("95");
        bl.close();
    }
...
MainWindow::~MainWindow() {
    if (m_rpmsgListener) {
        m_rpmsgListener->stop();
        m_rpmsgListener->wait();
    }
    if (m_processThread) {
...
void MainWindow::setupUi() {
    ...
    // -- LED 补光灯控制 (M4 PWM) --
    QGroupBox *ledGroup = new QGroupBox("LED 补光灯");
    QHBoxLayout *ledLayout = new QHBoxLayout(ledGroup);
    ledLayout->setSpacing(4);
    m_sliderLedBright = new QSlider(Qt::Horizontal);
    m_sliderLedBright->setRange(0, 100);
    m_sliderLedBright->setValue(10);
    m_lblLedValue = new QLabel("10%");
    ledLayout->addWidget(new QLabel("亮度:"));
    ledLayout->addWidget(m_sliderLedBright, 1);
    ledLayout->addWidget(m_lblLedValue);
    connect(m_sliderLedBright, &QSlider::valueChanged, [this](int val) {
        m_lblLedValue->setText(QString("%1%").arg(val));
        if (m_rpmsgListener)
            m_rpmsgListener->sendCommand(QString("L%1").arg(val));
    });
    debugLayout->addWidget(ledGroup);
    ...
    m_lblTemp->setStyleSheet("font-size: 17px;");
    btnRow2->addWidget(m_lblFps);
    btnRow2->addWidget(m_lblTemp);
...
void MainWindow::setupVideoSource() {
    ...
    connect(m_displayTimer, &QTimer::timeout, [this]() {
        ...
        // P7.1: 从 displayQueue 取 BGR Mat
        cv::Mat mat;
        if (m_frozen || !m_displayQueue.latest(mat)) return;
        m_latestRawMat = mat;
        m_frameCount++;
    ...
    // --- M4 RPMsg 通信 ---
    m_rpmsgListener = new RpmsgListener("/dev/ttyRPMSG0", this);

    connect(m_rpmsgListener, &RpmsgListener::temperatureUpdated, [this](double t) {
        m_lblTemp->setText(QString("温度: %1\u00b0C").arg(t, 0, 'f', 1));
        if (t > 41.0)
            m_lblTemp->setStyleSheet("font-size: 17px; color: red; font-weight: bold;");
        else
            m_lblTemp->setStyleSheet("font-size: 17px;");
    });
    connect(m_rpmsgListener, &RpmsgListener::humidityUpdated, [this](double h) {
        QString cur = m_lblTemp->text();
        if (cur.contains("\u00b0C"))
            m_lblTemp->setText(cur + QString(" / %1%").arg(h, 0, 'f', 1));
    });
    connect(m_rpmsgListener, &RpmsgListener::freezeRequested,
            this, &MainWindow::onToggleFreeze);
    connect(m_rpmsgListener, &RpmsgListener::captureRequested,
            this, &MainWindow::onCapturePhoto);
    connect(m_rpmsgListener, &RpmsgListener::alarmReceived, [this](int level) {
        if (level >= 2)
            m_lblStatus->setText("⚠ 温度严重超限! LED 已关闭");
        else if (level >= 1)
            m_lblStatus->setText("⚠ 温度警告, 亮度已降低");
    });
    connect(m_rpmsgListener, &RpmsgListener::connectionChanged, [this](bool ok) {
        if (ok)
            m_lblStatus->setText("M4 已连接");
    });

    m_rpmsgListener->start();

    // 启动后周期性发送初始化消息，触发 M4 的 endpoint ready
    QTimer *initTimer = new QTimer(this);
    connect(initTimer, &QTimer::timeout, [this, initTimer]() {
        static int retries = 0;
        if (m_rpmsgListener && m_rpmsgListener->sendCommand(QString("L%1").arg(m_sliderLedBright->value()))) {
            initTimer->stop();
            initTimer->deleteLater();
        } else if (++retries > 30) {  // 15 秒后放弃
            initTimer->stop();
            initTimer->deleteLater();
        }
    });
    initTimer->start(500);  // 每 500ms 重试
}
